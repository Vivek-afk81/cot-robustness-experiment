...
# def load_correct(path, condition_filter=None):
#     out = {}
#     with open(path) as f:
#         for line in f:
#             r = json.loads(line)
...
# baseline = load_correct("results/stage2_baseline_control_v2_trial1.jsonl")
# reversed_t2 = load_correct("results/stage2_results_v2_trial2.jsonl", "reversed")
# shuffled_t2 = load_correct("results/stage2_results_v2_trial2.jsonl", "shuffled")

# for pid in [48, 83]:
#     print(f"Reversed pid={pid}: baseline={baseline.get(pid)}, reversed_t2={reversed_t2.get(pid)}")
# for pid in [20, 50, 73, 11, 21, 96]:
#     print(f"Shuffled pid={pid}: baseline={baseline.get(pid)}, shuffled_t2={shuffled_t2.get(pid)}")

# def mcnemar_bc(a, b):
#     common = set(a) & set(b)
#     bb = sum(1 for p in common if a[p] and not b[p])
#     cc = sum(1 for p in common if not a[p] and b[p])
#     return bb, cc

# print("\nDirect recompute — Baseline vs Reversed (trial2):", mcnemar_bc(baseline, reversed_t2))
# print("Direct recompute — Baseline vs Shuffled (trial2):", mcnemar_bc(baseline, shuffled_t2))
